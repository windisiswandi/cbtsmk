...
<script>
    const loadData = () => {
        $.ajax({
            type: 'GET',
            url: 'mod_siswa/crud_siswa.php?pg=ambil_siswa',
            success: function(data) {
                const data_siswa = JSON.parse(data);
                let html = '';
                data_siswa.data.forEach((siswa, i) => {
                    html += `
                    <tr>
                        <th width='3px'>${++i}</th>
                        <th>${siswa.no_pendaftar}</th>
                        <th>${siswa.nama}</th>
                        <th>${siswa.level}</th>
            ...
            'columns': [{
                ...
                },
                {
                    ...
                    'mRender': function(data, type, row) {
                        let content = `<a class="btn btn-flat btn-xs status ${data=='aktif' ? 'btn-success':'bg-maroon'}" data-status="${data}" data-id="${row.id_siswa}">${data}</a>`;
                        return content;
                    }
                },
                <?php if ($pengawas['level'] == 'admin') { ?>
    ...
    $('#tabelsiswa').on('click', '.status', function() {
        const e = $(this);
        var id_siswa = e.data('id')
        var status = e.data('status');
        var toStatus = status == "aktif" ? "tidak":"aktif";
        $.ajax({
            url: 'mod_siswa/crud_siswa.php?pg=status',
            method: "POST",
            data: {id_siswa, toStatus},
            success: function(data) {
                iziToast.success({
                    title: 'Horee!',
                ...
                setTimeout(function() {
                    e.text(toStatus)
                    e.toggleClass('bg-maroon')
                    e.toggleClass('btn-success')
                    e.data('status', toStatus)
                }, 1);
            }
        });
    });
